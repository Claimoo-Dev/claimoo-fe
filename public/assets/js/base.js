...
function AddtoHome(time, once) {
    if (once) {
        var AddHomeStatus = localStorage.getItem("ClaimooAddtoHome");
        if (AddHomeStatus === "1" || AddHomeStatus === 1) {
            // already showed up
        }
        else {
            localStorage.setItem("ClaimooAddtoHome", 1)
...
//-----------------------------------------------------------------------
// Dark Mode
var checkDarkModeStatus = localStorage.getItem("ClaimooDarkmode");
var switchDarkMode = document.querySelectorAll(".dark-mode-switch");
var pageBodyActive = pageBody.classList.contains("dark-mode");

// Check if enable as default
if (Claimoo.Dark_Mode.default) {
    pageBody.classList.add("dark-mode");
}

// Local Dark Mode
if (Claimoo.Dark_Mode.local_mode.enable) {
    var nightStart = Claimoo.Dark_Mode.local_mode.start_time;
    var nightEnd = Claimoo.Dark_Mode.local_mode.end_time;
...
if (Claimoo.Dark_Mode.auto_detect.enable)
...
switchDarkMode.forEach(function (el) {
    el.addEventListener("click", function () {
        var darkmodeCheck = localStorage.getItem("ClaimooDarkmode");
        var bodyCheck = pageBody.classList.contains('dark-mode');
        if (darkmodeCheck === 1 || darkmodeCheck === "1" || bodyCheck) {
            pageBody.classList.remove("dark-mode");
            localStorage.setItem("ClaimooDarkmode", "0");
            switchDarkModeCheck(false);
        }
        else {
            pageBody.classList.add("dark-mode")
            switchDarkModeCheck(true);
            localStorage.setItem("ClaimooDarkmode", "1");
        }
    })
})
//-----------------------------------------------------------------------
...
function testMode() {
    ...
    console.clear();
    console.log("%cClaimoo", "font-size: 1.3em; font-weight: bold; color: #FFF; background-color: #6236FF; padding: 10px 120px; margin-bottom: 16px;")
    ...
    testModeMsg(Claimoo.PWA.enable, "PWA")
    testModeMsg(Claimoo.Dark_Mode.default, "Set dark mode as default theme")
    testModeMsg(Claimoo.Dark_Mode.local_mode.enable, "Local dark mode (between " + Claimoo.Dark_Mode.local_mode.start_time + ":00 and " + Claimoo.Dark_Mode.local_mode.end_time + ":00)")
    testModeMsg(Claimoo.Dark_Mode.auto_detect.enable, "Auto detect dark mode")
    testModeMsg(Claimoo.RTL.enable, "RTL")
    testModeMsg(Claimoo.Test.enable, "Test mode")
    testModeMsg(Claimoo.Test.alert, "Test mode alert")
    ...
    testModeMsg(Claimoo.Animation.goBack, "Go Back")
}
function themeTesting() {
    var word = Claimoo.Test.word;
    var value = "";
